...
public class ThingArtifact extends Artifact {
    private static final String WEBID_PREFIX = "";
    ...
    public List<String[]> getActionDataStructure() {
        List<String[]> actionMap = new ArrayList<>();

        // Iterate through all action affordances in the TD
        for (ActionAffordance action : td.getActions()) {
            String actionName = action.getName();
            //log(actionName);

            // Check if there is an input schema
            Optional<DataSchema> inputSchema = action.getInputSchema();

            if (inputSchema.isPresent() && inputSchema.get().getDatatype().equals(DataSchema.OBJECT)) {
                ObjectSchema objectSchema = (ObjectSchema) inputSchema.get();

                // Get all property names (tags) for this object
                for (String propertyName : objectSchema.getProperties().keySet()) {
                    actionMap.add(new String[] { actionName, propertyName });
                }
            } else {
                // If it's a simple action without an object payload, just add the name
                actionMap.add(new String[] { actionName, null });
            }
        }
            ...
            case DataSchema.OBJECT:
                ...
                if (tags.isPresent()) {
                    ...
                    for (String tag : payload.keySet()) {
                        tagList.add(tag.contains("#") ? tag.substring(tag.indexOf("#") + 1) : tag);
